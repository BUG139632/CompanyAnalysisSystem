...
class InnovationCapabilityDimension(AnalysisDimension):
    """创新能力分析维度"""
    def __init__(self):
        super().__init__(
            name="innovation_capability",
            description="综合分析公司的创新能力"
        )
    def get_prompt_template(self) -> str:
        return """
你是一位专业的公司财务分析师。请根据下方提供的财务指标数据，分析各项创新相关指标的年度变化、同比/环比、趋势等，并解释这些指标如何反映公司的创新能力（如研发投入、创新收入、专利数量等）。
...
1. **创新指标趋势分析**：分析各项创新相关财务指标的年度变化、同比/环比、趋势等。

2. **创新能力评价**：解释这些财务指标如何反映公司的创新能力，包括但不限于：研发投入、创新产出、创新收入、专利数量等。


请以如下JSON格式返回：
{{
  "analysis_result": "（包含创新指标趋势分析和创新能力评价的完整分析内容，不要出现可视化推荐部分）",
  "recommended_visualization_metrics": [["rd_expense", "innovation_revenue"], ["patent_count", "net_profit"] ...]
}}
"""
    def get_required_tags(self) -> List[str]:
        return ["innovation", "rd", "patent", "technology", "product", "management", "strategy", "culture", "incentive", "revenue"]
    def get_required_metrics(self) -> List[str]:
        return [
            "rd_expense", "rd_expense_ratio", "innovation_revenue", "innovation_revenue_ratio", "patent_count",
...
class InnovationCapabilityAnalyzer:
    ...
    def __init__(self):
        ...
        innovation_dimension = InnovationCapabilityDimension()
        self.llm_analyzer.add_custom_dimension(innovation_dimension)
    ...
    def analyze_innovation_capability(self) -> Dict[str, Any]:
        try:
            self.logger.info("开始分析创新能力")
            self._ensure_data_loaded()
            if not self._financial_data:
                return {"error": "没有找到财务数据"}
            # 只传递财务数据，文档数据传空列表
            result = self.llm_analyzer.analyze_dimension("innovation_capability", self._financial_data, [])
            if "error" in result:
                self.logger.error(f"分析失败: {result['error']}")
                return result
            self._last_analysis_result = result
            result["analyzer_type"] = "InnovationCapabilityAnalyzer"
            ...
            self.logger.info("创新能力分析完成")
            return result
        except Exception as e:
            self.logger.error(f"分析创新能力时发生错误: {str(e)}")
